...
namespace DigitalOpus.MB.MBEditor
{
    ...
    public class MB3_MeshBakerEditor : Editor
    {
        ...
        [MenuItem("Tools/Custom/Custom Baker", false, 100)]
        public static GameObject Custom_CreateNewMeshBaker()
        {
            //Set object name in hierarchy
            GameObject g = CreateNewMeshBaker();
            g.name = "CustomBaker" + ExtractMeshBakerInstanceNumber(g.name);

            //Handle setting the Texture Baker component to correct settings
            MB3_TextureBaker tb = g.GetComponent<MB3_TextureBaker>();
            tb.resultType = MB2_TextureBakeResults.ResultType.textureArray;

            //Handle setting the Mesh Baker component to correct settings
            MB3_MeshBakerGrouper mbg = g.GetComponent<MB3_MeshBakerGrouper>();
            mbg.data.clusterByLODLevel = true;
            mbg.GetMeshBakerSettings().lightmapOption = MB2_LightmapOptions.preserve_current_lightmapping;
            Selection.activeGameObject = g;
            return g;
        }
        
        /// <summary>
        /// Extracts the number of the current Mesh Baker instance (Eg (1) (2) etc)
        /// </summary>
        /// <param name="currentName"></param>
        /// <returns></returns>
        private static string ExtractMeshBakerInstanceNumber(string currentName) //TODO : Consider moving to its own static class?
        {
            int fileNameStartIndex = currentName.LastIndexOf("(")-1;
            string fileName = currentName.Substring(fileNameStartIndex, currentName.Length - fileNameStartIndex);
            return fileName;
        }
