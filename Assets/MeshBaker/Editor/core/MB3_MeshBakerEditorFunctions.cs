...
public class MB3_MeshBakerEditorFunctions
{
    ...
    private static bool _BakeIntoCombinedSceneObject(MB3_MeshBakerCommon mom, bool createdDummyTextureBakeResults, ref SerializedObject so)
    {
        ...
        if (mom.AddDeleteGameObjects(mom.GetObjectsToCombine().ToArray(), null, false))
        {
            ...
            //Allows both use of unmodified MeshBaker and the Custom version. Only do  custom setup when specifically batching by LOD level
            if (mom.GetTextureBaker() != null &&
                mom.GetTextureBaker().gameObject.GetComponent<MB3_MeshBakerGrouper>().grouper.d.clusterByLODLevel == true)
            {
                CustomManageGeneratedCombinedMeshes(mom);
            }
    ...
    /// <summary>
    /// Manages taking the combined meshes generated by Mesh Baker, and childs them to a newly generated parent object with a newly generated LOD group
    /// </summary>
    /// <param name="mom"></param>
    private static void CustomManageGeneratedCombinedMeshes(MB3_MeshBakerCommon mom)
    {
        string newGOwithLOD = "LOD_CONTAINER_" + mom.GetTextureBaker().name;
        
        GameObject lodContainer = GameObject.Find(newGOwithLOD);

        if (lodContainer == null)
        {
            lodContainer = new GameObject();
            lodContainer.name = newGOwithLOD;
        }

        if (lodContainer.GetComponent<LODGroup>() == null)
        {
            lodContainer.AddComponent<LODGroup>();
        }

        lodContainer.GetComponent<LODGroup>().fadeMode = LODFadeMode.None;

        mom.parentSceneObject = lodContainer.transform;
        mom.meshCombiner.resultSceneObject.transform.parent = mom.parentSceneObject;

        Transform childRef = mom.meshCombiner.resultSceneObject.transform.GetChild(0);
        childRef.parent = mom.parentSceneObject;
        GameObject.DestroyImmediate(mom.meshCombiner.resultSceneObject);
        mom.meshCombiner.resultSceneObject = childRef.gameObject;


        //Explicitly set the Renderer lightmap data to be that of the source objects lightmap data. 
        var meshBakerLMDH = mom.meshCombiner.resultSceneObject.AddComponent<LightmapDataHolder>();
        var renderer = mom.meshCombiner.resultSceneObject.GetComponent<MeshRenderer>();

        LightmapDataHolder.MeshRendererData meshRendererData = new LightmapDataHolder.MeshRendererData(renderer);
        SerializedObject srcSob = new SerializedObject(renderer);

        SerializedProperty m_LightmapIndex = srcSob.FindProperty("m_LightmapIndex");
        meshRendererData.lightmapIndex = m_LightmapIndex.intValue;

        SerializedProperty m_LightmapTilingOffset = srcSob.FindProperty("m_LightmapTilingOffset");
        meshRendererData.tilingAndOffset = m_LightmapTilingOffset.vector4Value;

        SerializedProperty m_ScaleInLightmap = srcSob.FindProperty("m_ScaleInLightmap");
        meshRendererData.scaleInLightmap = m_ScaleInLightmap.floatValue;
        
        srcSob.ApplyModifiedProperties();

        meshBakerLMDH.meshRendererData = meshRendererData;
        EditorUtility.SetDirty(renderer);
        EditorUtility.SetDirty(meshBakerLMDH);
        
        //--------------------- Start assigning the LOD data to the LOD group ----------------//
        int childCount = lodContainer.transform.childCount;
        LODGroup lodGroup = lodContainer.GetComponent<LODGroup>();
        LOD[] lods = lodGroup.GetLODs();
        for (int i = 0; i < childCount; i++)
        {
            List<Renderer> renderers = new List<Renderer>();
            if (!renderers.Contains(lodContainer.transform.GetChild(i).GetComponent<Renderer>()))
            {
                renderers.Add(lodContainer.transform.GetChild(i).GetComponent<Renderer>());
        ...
        //Sort out the percentages of the LODS. Cant do this directly without accessing a serialized property 
        ...
        for (int j = 0; valArrProp.arraySize > j; j++)
        {
            ...
            if (j == 0) { sHeight.doubleValue = 0.4f; } //LOD 1

            if (j == 1) { sHeight.doubleValue = 0.2f; }

            if (j == 2) { sHeight.doubleValue = 0.02f; }
        }
