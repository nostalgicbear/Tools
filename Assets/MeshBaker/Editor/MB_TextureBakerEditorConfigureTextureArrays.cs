...
namespace DigitalOpus.MB.MBEditor
{
    ...
    public class MB_TextureBakerConfigureTextureArrays
    {
        ...
        public static void DrawTextureArrayConfiguration(MB3_TextureBaker momm, SerializedObject textureBaker, MB3_TextureBakerEditorInternal editorInternal)
        {
            ...
            if (GUILayout.Button(new GUIContent("Custom: " + MB3_TextureBakerEditorInternal.configAtlasTextureSlicesFromObjsContent.text),style))
            {
                ConfigureTextureArraysFromObjsToCombine(momm, editorInternal.resultMaterialsTexArray, textureBaker, true);
            }
            ...
            if (GUILayout.Button("Toggle Consider Mesh UVs"))
            {
                ToggleConsiderMeshUVs(momm);
            }
        ...
        /// <summary>
        /// Toggles the ConsiderMeshUVs option for ALL entries in this mesh baker MaterialTextureArray.
        /// </summary>
        /// <param name="mom"></param>
        public static void ToggleConsiderMeshUVs(MB3_TextureBaker mom)
        {
            if (mom == null || mom.resultMaterialsTexArray.Length == 0)
            {
                Debug.Log("You must build texture array slices before you can toggle this option");
                return;
            }
            
            foreach (MB_MultiMaterialTexArray entry in mom.resultMaterialsTexArray)
            {
                foreach (MB_TexArraySlice slice in entry.slices)
                {
                    slice.considerMeshUVs = !slice.considerMeshUVs;
                }
            }
        }
        ...
        public static void ConfigureTextureArraysFromObjsToCombine(MB3_TextureBaker mom, SerializedProperty resultMaterialsTexArrays, SerializedObject textureBaker, bool separateSlices = false)
        {
            ...
            //ONLY VALIDATION BEING DONE TO HERE 
            ...
            foreach (MB3_TextureBakerEditorInternal.MultiMatSubmeshInfo resMatKey in shader2ResultMat_map.Keys)
            {
                ...
                if (separateSlices)
                {
                    //Separate out slices in order to add to texture array

                    List<MB_TexArraySlice> listCopy = mm.slices;

                    foreach (var texSlice in listCopy.ToList())
                    {
                        for (int i = 0; i < texSlice.sourceMaterials.Count; i++)
                        {
                            MB_TexArraySlice resSlice = new MB_TexArraySlice();
                            resSlice.considerMeshUVs = true; //Forcefully setting to TRUE rather than taking from original (to avoid undesired result)

                            resSlice.sourceMaterials.Add(new MB_TexArraySliceRendererMatPair()
                            {
                                renderer = texSlice.sourceMaterials[i].renderer,
                                sourceMaterial = texSlice.sourceMaterials[i].sourceMaterial
                            });

                            listCopy.Add(resSlice);
                        }
                    }

                    listCopy.RemoveAt(0); //Remove first element as its only used as a reference for building the above
                    mm.slices = listCopy;
                }
