...
namespace DigitalOpus.MB.MBEditor
{
    ...
    public class MB3_MeshBakerGrouperEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            //GUILayout.Button("Custom: Generate Mesh Bakers w/Filters")
            
            var style = new GUIStyle(GUI.skin.button);
            style.normal.textColor = Color.green;
            
            if (GUILayout.Button(new GUIContent("Custom: Generate Mesh Bakers w/Filters",
                "Removes objects without renderer at LOD 0 \n" +
                "Removes objects with LOD level > 2 \n" +
                "Removes objects without a LOD group \n" +
                "Adds LOD levels for objects missing LOD1 and LOD2 \n"), style))
            {
                for(int tIdx = 0; tIdx < targets.Length; tIdx++) //TARGET is texture baker obj
                {
                    _generateMeshBakers(targets[tIdx], true);
                }
            }
        ...
        private static void _generateMeshBakers(UnityEngine.Object target, bool applyFilters = false)
        {
            MB3_MeshBakerGrouper tbg = (MB3_MeshBakerGrouper)target;
            MB3_TextureBaker tb = tbg.GetComponent<MB3_TextureBaker>();
            if (tb == null)
            ...
            if (proceed)
            {
                ...
                if (applyFilters)
                {
                    foreach (var baker in newBakers)
                    {
                        baker.objsToMesh = CustomTools.MeshBakerUtilities.Custom_MeshBakerUtility.RemoveObjectsWithIncorrectLODs(baker.objsToMesh);
                        if (baker.objsToMesh.Count == 0) { DestroyImmediate(baker.gameObject); }
                    }
        ...
        private static void _enableDisableRenderers(UnityEngine.Object target, bool enableRenderers)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < mBakers.Length; i++)
                {
                    ...
                    ApplyCustomToggleFunctionality(enableRenderers, mBakers, i);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Toggles the generated-combined mesh to be the opposite of the source mesh. 
        /// </summary>
        /// <param name="enableRenderers">whether renderer should be on/off</param>
        /// <param name="mBakers">The list of bakers to iterate over (there can be many bakers to a baker grouper)</param>
        /// <param name="i">index</param>
        private static void ApplyCustomToggleFunctionality(bool enableRenderers, MB3_MeshBakerCommon[] mBakers, int i)
        {
            int childCount = mBakers[i].meshCombiner.resultSceneObject.transform.childCount;

            if (childCount == 0) //There are no children, meaning this is likely an a custom object I adapted the hierarchy of for the purpose of supporting LODs
            {
                mBakers[i].meshCombiner.resultSceneObject.GetComponent<Renderer>().enabled = !enableRenderers;
            }
            else
            {
                for (int j = 0; j < mBakers[i].meshCombiner.resultSceneObject.transform.childCount; j++)
                {
                    Renderer combinedMeshChildRenderer =
                        mBakers[i].meshCombiner.resultSceneObject.transform.GetChild(j).GetComponent<Renderer>();
                    if (combinedMeshChildRenderer != null)
                    {
                        combinedMeshChildRenderer.enabled = !enableRenderers;
                    }
                }
            }
        }
