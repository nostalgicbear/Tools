...
namespace DigitalOpus.MB.MBEditor
{
    public class MB3_MeshBakerEditorWindowAddObjectsTab : MB3_MeshBakerEditorWindowInterface
    {
        ...
        public enum LIGHTMAP_OPTIONS
        {
            DONT_FILTER_BY_LIGHTMAP = -2,
            NOT_LIGHTMAPPED = -1,
            HAS_LIGHTMAP = 0,
            HAS_SPECIFIC_LIGHTMAP_INDEX = 1
        }

        public LIGHTMAP_OPTIONS LightmapOptions = LIGHTMAP_OPTIONS.DONT_FILTER_BY_LIGHTMAP;
        ...
        public void drawTabAddObjectsToBakers()
        {
            ...
            // lightmapDisplayValues[2] = "has lightmap";
            ...
            LightmapOptions = (LIGHTMAP_OPTIONS)EditorGUILayout.EnumPopup("Lightmap Option: ", LightmapOptions);
            
            if (LightmapOptions == LIGHTMAP_OPTIONS.HAS_SPECIFIC_LIGHTMAP_INDEX)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Using Lightmap Index ");
                lightmapIndex = EditorGUILayout.IntSlider(lightmapIndex, 0, 256);
                EditorGUILayout.EndHorizontal();
            }
            else
            {
                lightmapIndex = (int) LightmapOptions;
            }
            ...
            {
                Debug.Log("Lightmnap seleted is : " + lightmapIndex);
                addSelectedObjects();
            }
            ...
            for (int j = 0; j < mrss.Count; j++)
            {
                ...
                if (mrs is MeshRenderer || mrs is SkinnedMeshRenderer)
                {
                    ...
                    if (!newMomObjs.Contains(mrs.gameObject))
                    {
                        ...
                        switch (LightmapOptions)
                        {
                            case LIGHTMAP_OPTIONS.DONT_FILTER_BY_LIGHTMAP:
                                //Dont have to add anythig as addMe = true by default
                                break;
                            case LIGHTMAP_OPTIONS.NOT_LIGHTMAPPED:
                                if (mrs.lightmapIndex != -1)
                                {
                                    numLightmapExcluded++;
                                    addMe = false;
                                }
                                break;
                            case LIGHTMAP_OPTIONS.HAS_LIGHTMAP:
                                if (mrs.lightmapIndex == -1)
                                {
                                    numLightmapExcluded++;
                                    addMe = false;
                                ...
                                break;
                            case LIGHTMAP_OPTIONS.HAS_SPECIFIC_LIGHTMAP_INDEX:
